...
public class UsePassesJobConfig {
    ...
    public UsePassesJobConfig(PassRepository passRepository, BookingRepository bookingRepository) {
        this.passRepository = passRepository;
        this.bookingRepository = bookingRepository;
    }

    @Bean
    public Job usePassesJob(JobRepository jobRepository, PlatformTransactionManager transactionManager, EntityManagerFactory entityManagerFactory) {
        return new JobBuilder("usePassesJob", jobRepository)
                .start(usePassesStep(jobRepository, transactionManager, entityManagerFactory))
                .build();
    }

    @Bean
    public Step usePassesStep(JobRepository jobRepository, PlatformTransactionManager transactionManager, EntityManagerFactory entityManagerFactory) {
        return new StepBuilder("usePassesStep", jobRepository)
                .<BookingEntity, Future<BookingEntity>>chunk(CHUNK_SIZE, transactionManager)
                .reader(usePassesItemReader(entityManagerFactory))
                .processor(usePassesAsyncItemProcessor())
                .writer(usePassesAsyncItemWriter(entityManagerFactory))
                .build();

    }

    @Bean
    public JpaCursorItemReader<BookingEntity> usePassesItemReader(EntityManagerFactory entityManagerFactory) {
    ...
    @Bean
    public AsyncItemWriter<BookingEntity> usePassesAsyncItemWriter(EntityManagerFactory entityManagerFactory) {
